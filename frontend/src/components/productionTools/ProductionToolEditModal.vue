...
const handleSubmit = async () => {
  ...
  try {
    if (form.value.tool_no) {
      await productionToolsStore.updateProductionTool(form.value.tool_no, form.value)
    } else {
    ...
    console.error('Error saving production tool:', error)
    alert(`Failed to save production tool: ${error.response?.data?.message || error.message}`)
  }
}
</script>
...
<template>
  <Dialog :open="open" @update:open="(val) => emit('update:open', val)">
    <DialogContent class="sm:max-w-[625px]">
      <DialogHeader>
        <DialogTitle>Edit Production Tool</DialogTitle>
        <DialogDescription>Update production tool details below</DialogDescription>
      </DialogHeader>
